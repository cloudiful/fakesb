...
fakESB publishes Linux GNU binaries and multi-architecture Docker images from the single GitHub Actions workflow at `.github/workflows/docker-publish.yml`.
...
The workflow publishes these binary archives:

- `fakesb-v0.1.0-x86_64-unknown-linux-gnu.tar.gz`
- `fakesb-v0.1.0-aarch64-unknown-linux-gnu.tar.gz`

Each archive contains `fakESB` and `SHA256SUMS`. Docker images are published to `ghcr.io/<owner>/<repository>` with version and `latest` tags. Both tags are multi-architecture manifests for `linux/amd64` and `linux/arm64`.
