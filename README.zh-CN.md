...
- 按任意 HTTP 方法和路径匹配，支持查询参数、请求头和正文匹配
  （JSON 字段路径与 XML XPath 表达式）。
- 返回模板化响应（`{{ req.* }}`、`{{ resp.* }}`、`{{ now }}`、`{{ uuid }}` 等），
  或将请求转发到配置的目标地址，并可选地用模板改写上游响应。
- 模拟延迟与有状态序列：规则可配置响应延迟，或按顺序循环返回一组响应步骤。
- 使用 PostgreSQL 保存请求、响应和快照，支持筛选、单条删除与批量清理。
- 在 Web 控制台直接试跑规则匹配，无需先发真实流量。
- 整个配置（目标、规则、模板）可一键 JSON 导入/导出，方便多环境共享。
